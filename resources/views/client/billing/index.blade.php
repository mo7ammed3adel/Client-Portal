<x-app-layout>
    <x-slot name="header">
        <div>
            <h1 class="portal-title">المدفوعات</h1>
            <p class="portal-muted mt-1">ادفع بأمان من خلال كاشير وتابع سجل المدفوعات.</p>
        </div>
    </x-slot>
    ...
    <div class="portal-container">
        ...
        <div class="grid gap-4 md:grid-cols-2">
            <x-metric-card label="إجمالي المبلغ المدفوع" :value="number_format($total, 2).' ج.م'" />
            <x-metric-card label="المبلغ المتبقي" :value="number_format($remaining, 2).' ج.م'" />
        </div>
        ...
        <div class="portal-card mt-6">
            <div class="portal-card-body">
                <div class="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        <h2 class="font-bold text-slate-950 dark:text-white">الدفع من خلال كاشير</h2>
                        <p class="portal-muted mt-1">اكتب المبلغ المطلوب دفعه. المدفوعات الناجحة تضاف إلى إجمالي المبلغ المدفوع.</p>
                    </div>

                    <form method="POST" action="{{ route('client.billing.pay.new') }}" class="grid w-full gap-3 sm:grid-cols-[1fr_auto_auto] lg:max-w-2xl">
                        @csrf
                        <div>
                            <label for="amount" class="sr-only">المبلغ</label>
                            <input id="amount" name="amount" type="number" min="1" step="0.01" value="{{ old('amount') }}" class="portal-input mt-0" placeholder="المبلغ" required>
                        ...
                        <button class="portal-button h-11" type="submit">ادفع الآن</button>
                    </form>
                </div>
            </div>
        </div>
        ...
        <div class="portal-card mt-6 overflow-hidden">
            <div class="portal-card-body border-b border-slate-200 dark:border-slate-800">
                <h2 class="font-bold text-slate-950 dark:text-white">سجل المدفوعات</h2>
            </div>
            @if ($invoices->isEmpty())
                <div class="p-8 text-center text-sm text-slate-500">لا توجد مدفوعات حتى الآن.</div>
            @else
                                ...
                                <tr>
                                    ...
                                    <td>
                                        @if ($invoice->status === 'paid')
                                            <span class="text-xs font-semibold text-slate-500">تم الدفع</span>
                                        @else
                                            <form method="POST" action="{{ route('client.billing.pay', $invoice) }}" class="flex flex-wrap items-center gap-2">
                                                ...
                                                <button class="portal-button px-3 py-2 text-xs" type="submit">ادفع بكاشير</button>
                                            </form>
                                        @endif
                                    </td>
                                </tr>
