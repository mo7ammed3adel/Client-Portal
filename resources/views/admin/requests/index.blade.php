<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="portal-muted mt-1">راجع طلبات العملاء وحدّث حالة التنفيذ.</p>
        </div>
    </x-slot>
    ...
    <div class="portal-container">
        ...
        @if ($tasks->isEmpty())
            <x-empty-state title="لا توجد طلبات توصيل" message="طلبات التوصيل من العملاء ستظهر هنا." />
        @else
                        ...
                        <div class="portal-card-body">
                            <div class="flex items-start justify-between gap-4">
                                <div>
                                    ...
                                    <p class="mt-1 text-sm text-slate-500">{{ $task->client->name }} · {{ $task->created_at->format('Y/m/d') }}</p>
                                </div>
                            ...
                            <form method="POST" action="{{ route('admin.requests.update', $task) }}" class="mt-5 flex flex-wrap items-center gap-3">
                                ...
                                <select name="status" class="portal-input mt-0 w-48">
                                    ...
                                    <option value="completed" @selected($task->status === 'completed')>مكتمل</option>
                                </select>
                                <button class="portal-button-secondary">تحديث</button>
                            </form>
                        </div>
                    </div>
                @endforeach
